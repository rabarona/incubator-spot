...
class SpotLDAWrapperTest extends TestingSparkContextFlatSpec with Matchers {
  ...
  "SparkLDA" should "handle distinct docs on distinct words with Online optimizer" in {
    ...
    "formatSparkLDAInput" should "return input in RDD[(Long, Vector)] (collected as Array for testing) format. The index " +
      ...
      val sparkLDAInput: RDD[(Long, Vector)] = SpotLDAWrapper.formatSparkLDAInput(documentWordData, documentDictionary,
        wordDictionary, sqlContext)
      val sparkLDAInArr: Array[(Long, Vector)] = sparkLDAInput.collect()

      sparkLDAInArr shouldBe Array((0, Vectors.sparse(4, Array(0, 3), Array(8.0, 5.0))), (2, Vectors.sparse(4, Array(2),
        Array(2.0))), (1, Vectors.sparse(4, Array(1), Array(4.0))))
    }
    ...
    "formatSparkLDADocTopicOuptut" should "return RDD[(String,Array(Double))] after converting doc results from vector: " +
      ...
      val docTopicDist: RDD[(Long, Vector)] =
        sparkContext.parallelize(Array((0.toLong, Vectors.dense(0.15, 0.3, 0.5, 0.05)), (1.toLong,
      ...
      val documents = sparkDocRes.select(DocumentName)
        .map(documentName => documentName.toString.replaceAll("\\[", "").replaceAll("\\]", ""))
        .collect()
    ...
    "formatSparkLDAWordOutput" should "return Map[Int,String] after converting word matrix to sequence, wordIDs " +
      "back to strings, and sequence of probabilities to array" in {
      val testMat = Matrices.dense(4, 4, Array(0.5, 0.2, 0.05, 0.25, 0.25, 0.1, 0.15, 0.5, 0.1, 0.4, 0.25, 0.25,
        0.7, 0.2, 0.02, 0.08))

      val wordDictionary = Map("-1_23.0_7.0_7.0_4.0" -> 3, "23.0_7.0_7.0_4.0" -> 0, "333333.0_7.0_7.0_4.0" -> 2,
        "80.0_7.0_7.0_4.0" -> 1)
